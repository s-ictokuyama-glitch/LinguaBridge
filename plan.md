...
| 配布 | `start.bat`（ダブルクリックのみ・N-09）が初回起動時に自動で `setup.ps1` を呼ぶ。以降は起動のみ（モデルDLはネットワーク要のため毎回は行わない）。`setup.ps1` は単独でも実行可（管理者実行でファイアウォール/スリープ設定まで自動化） | 運用先は実質1台。PyInstaller化はネイティブ依存とモデル同梱で巨大化・破損しやすく不採用 |
...
├── setup.ps1                     # 初回セットアップ（要管理者権限。単独実行も、start.bat からの自動実行も可）
...
20. 運用スクリプト完成: `setup.ps1`（Python導入→venv→pip→モデルDL→証明書→ファイアウォール規則→電源スリープ無効化 `powercfg`）、`start.bat`（`.venv`が無ければ初回のみ `setup.ps1` を自動実行→起動→コード生成→既定ブラウザで先生ページを開く）
