...
- **QRを読むと `https://` で開いてしまい「安全な接続ではありません」になる**:
  最近のブラウザ（Chrome/Edge/Safari）とQRリーダーアプリには、`http://` を自動で `https://` に
  上げる機能があります。サーバー側から止めることはできないので、次の順に対処してください。
  1. **その場でしのぐ**: 警告画面の「詳細設定 →（このまま）続行」を押す。または先生ページに
     表示されている `http://…:8000/?code=…` を、生徒にブラウザのアドレス欄へ手入力してもらう
     （QRを介さなければ自動アップグレードは起きにくい）。参加コードだけ伝えて手入力でも参加できます。
  2. **端末側の設定を切る**（毎回起きる端末はこれが原因）: Chrome/Edge は
     設定 →「プライバシーとセキュリティ」→「常に安全な接続を使用する」をオフ。
     管理Chromebook等でユーザーが変更できない場合は、管理者が Chrome ポリシー
     `HttpsOnlyMode` を無効、または `HttpAllowlist` に `192.168.*`（実際のLANの範囲）を追加。
  3. **QRのホスト名を変える**: `config.yaml` の `server.public_host` に mDNS名
     （例 `linguabridge.local`。サーバーPCのホスト名 + `.local`）を設定すると、ブラウザは
     この種の「非一意ホスト名」をアップグレード対象から除外するので改善することがあります。
     ただしmDNS名を引けない端末があるので、事前に1台で疎通確認してから採用してください。
  4. **どうしても直らない環境**: `config.yaml` の `server.join_scheme` を `https` にすると、
     QR・参加URLごと HTTPS（8443）で配ります。アップグレードされても行き先が同じなので確実に
     つながりますが、生徒端末それぞれで自己署名証明書の警告を1回承認する必要があります
     （`python scripts\make_cert.py --force` で証明書を作り直してから使ってください）。
