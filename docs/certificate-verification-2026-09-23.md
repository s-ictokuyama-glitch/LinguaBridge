...
実モデルを読み込む `test_real_mt.py`・`test_real_asr.py` は、#38の記録に倣って除外した（本変更と無関係）。
追記: 同日のその後の再実行では、この2ファイルを含めて1013件成功・5件スキップ（213秒）で、制限時間は超えなかった。
ディスクキャッシュが冷えた初回などにモデル読込が20秒を超えると、試験プロセスごと終了する危険がある。
そのため、本追記と同じコミットで2ファイルの制限時間を180秒にした。
